...
FASTAまたはGenBank形式のプラスミド配列を入力し、指定したカバレッジを満たすように既存プライマーを選択し、不足する場合は新規プライマーを設計します。
...
4. `Target coverage` を選択します。通常はデフォルトの `2` を使います。
...
設計が成功したか、最低カバレッジ、使用プライマー数を確認できます。プライマーリストとカバレッジレポートを `.txt` としてダウンロードできます。

### Coverage

各塩基位置のカバレッジを線グラフで表示します。赤い領域があれば、指定カバレッジを満たしていない不足領域です。
...
マスク、ユニーク性、Tm/GC、プライマー間隔の制約により、指定カバレッジを達成できないことがあります。その場合でも、ツールは最大限の候補セットと不足領域を返します。
